...
create_lib -ref_lib $PDK_PATH/lib/ndm/saed32rvt_c.ndm 8BIT_SHIFTER_LIB

read_verilog {./../DC/results/shifter.mapped.v} -library 8BIT_SHIFTER_LIB -design 8bit_shifter -top 8bit_shifter

#open the lib and block after saving
open_lib 8BIT_SHIFTER_LIB
open_block 8initialize_floorplan -core_utilization 0.6 -shape T -orientation S -core_offset 2 -flip_first_row true
set_individual_pin_constraints -offset {1 20} -sides 8 -ports [get_ports]
place_pins -self
create_placement -floorplanBIT_SHIFTER
...
set_individual_pin_constraints -ports [get_ports Clock] -exclude_sides {1 2 3}
place_pins -self
